...
export async function GET(request: NextRequest) {
  try {
    ...
    // Log raw headers and URL so we can see what Meta sends (Netlify will log these)
    const headers: Record<string, string> = {};
    for (const [k, v] of request.headers) {
      headers[k] = v;
    }
    console.log('[FB WEBHOOK] Incoming GET verification. url:', reqUrl.toString());
    console.log('[FB WEBHOOK] headers:', headers);
    console.log('[FB WEBHOOK] query:', { mode, tokenProvided: !!token, challengePresent: !!challenge });
